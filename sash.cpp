...
#define DEGREE 1//1 for lineal, 2 for cube

double EPS = 1e-16;
double X_BEGIN = 3.0;
double X_END = 22.0;
size_t ELEMS_NUM = 40; //20 or 40
double L = (X_END - X_BEGIN) / ELEMS_NUM;

double a = 13.0, B = -7.0, C = 0.0, D = 7.0, usl_left = 5.0, usl_right = 5.0; // au"+Bu'+Cu+D=0
...
double analytical_solution(double x) {
    return ((-1) * exp(21. / 13.) * (x - 17.) - 19. * exp((7. * x) / 13.) + exp(154. / 13.) * (x + 2.)) / (exp(21. / 13.) * (exp(133. / 13.) - 1.));
}
...
int main() {
    ...
    if (DEGREE == 1) {
     ...
     if (DEGREE == 1) {
        if(ELEMS_NUM == 20) {
            tab = fopen("lin_20.txt", "w");
        }
        if(ELEMS_NUM == 40) {
            tab = fopen("lin_40.txt", "w");
        }
     } else {
        if(ELEMS_NUM == 20) {
            tab = fopen("cub_20.txt", "w");
        }
        if(ELEMS_NUM == 40) {
            tab = fopen("cub_40.txt", "w");
        }
     }
